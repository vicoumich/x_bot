...
TWEET_KEYS = [
    ...
    "in_reply_to",
    "is_quote_status",
    "quote",  # Correspond à "quote_id" en SQL
    "retweeted_tweet",  # Correspond à "retweeted_tweet_id" en SQL
    "possibly_sensitive",
...
USER_KEYS = [
    ...
    "location",
    "followers_count",
    "following_count",  # Remplace "friends_count" de la table par sa clé réelle
    "statuses_count",
    "favourites_count",
    "created_at",
    "verified",
    "profile_image_url",  # Commentée dans `user_info`, peut être ajoutée si activée
    "profile_banner_url",  # Idem
    "last_tweet_id",  # Non défini dans `user_info`, peut être calculé ailleurs
    "last_active_at",  # Non défini dans `user_info`
    "protected"
]
...
class UserDatabaseManager:
    ...
    async def add_user(self, user):
        ...
        await self.conn.commit()
        id = user["id"]
        logging.info(f"User ID {id} added to database.")


    async def get_user_by_username(self, username=None):
        """
        Retrieve user from the database, optionally filtered by username.

        :param username: The username of the user to retrieve.
        :return: The user profile with the username.
        """
        ...
        cursor = await self.conn.execute(query, params)
        return [dict(zip(USER_KEYS, row)) for row in await cursor.fetchall()]

    async def get_user_by_id(self, user_id=None):
        """
        Retrieve user from the database, optionally filtered by user id.
